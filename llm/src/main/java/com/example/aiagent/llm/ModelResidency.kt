...
/**
 * What to warm at startup: an engine, the request to load it with, and enough about the model to
 * check it will fit before trying.
 *
 * A value rather than a callback so the host does the deciding eagerly and residency does only the
 * loading -- which is what lets this file know nothing about chats, settings or catalogues.
 */
data class WarmUp(
 ...
 *
 * It is warmed once at startup ([preload]) and thereafter handed to every chat through [open]. A
 * fresh chat whose load matches the resident model reuses it with a cheap conversation reset; a
 * resumed conversation, a different model, or changed settings loads normally and becomes the new
 * resident. Nothing unloads when a chat closes -- that is the whole point -- so an engine holds one
...
class ModelResidency(
    /**
     * Read lazily. The probe touches `ActivityManager` and is cheap but not free, and residency is
     * constructed as part of the host's object graph -- a lambda keeps it off the construction path
     * and keeps this file free of a DI framework's `Provider`.
     */
    ...
    /**
     * Kicks off the background warm-load of [warmUp]. Non-blocking; call once, at startup.
     *
     * *Which* model to warm, and what request to warm it with, is the host's decision and is
     * computed by the caller -- in this app, from the active model in Settings and the plan a fresh
     * chat would build, system prompt included, so the warmed request is byte-identical to the one
     * `ChatViewModel` asks for and the chat reuses the resident engine instead of reloading it.
     * Residency used to reach for `ChatLoadPlanner` and `SettingsStore` to work that out itself,
     * which tied the one genuinely reusable piece of model hosting to this app's chat screen.
     *
     * Null does nothing, which is the honest answer for a host with nothing worth warming.
     */
    fun preload(warmUp: WarmUp?) {
        ...
        try {
            ...
            val fit = ModelFitEvaluator.evaluate(
                model = warmUp.model,
                engine = warmUp.engine.descriptor,
                accelerator = warmUp.accelerator,
                device = deviceMemory(),
                isDownloaded = true,
            )
            ...
            preloadJob = scope.launch {
                runCatching {
                    loadResident(warmUp.engine, warmUp.request, freshRequest = warmUp.request)
