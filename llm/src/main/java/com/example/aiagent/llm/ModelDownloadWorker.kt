 ...
 *
 * A plain constructor, not `@HiltWorker`. WorkManager instantiates workers itself, so a worker with
 * injected dependencies needs *some* factory -- and `@HiltWorker` makes that factory Hilt's, which
 * would force every consumer of this library to adopt Hilt to download a model. [LlmWorkerFactory]
 * is the neutral answer; a host that does use Hilt composes it in, which is what `:app` does.
 */
