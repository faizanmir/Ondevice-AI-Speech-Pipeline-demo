...
sealed interface ModelLoadPlan {
    ...
    /**
     * No registered engine can load this format *on this device*.
     *
     * This used to be the GGUF-without-llama.cpp case. Now it is reached when LiteRT-LM reports
     * itself unavailable -- its native library missing, or the chipset unsupported -- which is a
     * device condition rather than a build one, so the branch stays.
     */
...
class ModelLoadPlanner(
    ...
    private val config: LlmConfigSource,
    private val engines: EngineRegistry,
    private val modelRepository: ModelRepository,
    private val paths: LlmPaths,
) {
    ...
    fun plan(modelId: String): ModelLoadPlan {
        ...
        return ModelLoadPlan.Resolved(
            ...
            nativeLibraryDir = paths.nativeLibraryDir,
