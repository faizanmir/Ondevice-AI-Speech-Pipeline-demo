...
class SpeakerDiarizer {
    ...
    /** Set instead of [diarization] when the loaded engine is [DiarizationEngine.FRAME]. */
    private var frame: FrameDiarizer? = null

    val isLoaded: Boolean get() = diarization != null || frame != null

    /**
     * Stretches where two people were talking at once, from the last run.
     *
     * Always empty on [DiarizationEngine.SHERPA], which has no way to know: overlapping speech is in
     * the frame posteriors its native call consumes and discards. Empty is the honest answer there
     * rather than a missing capability the caller has to branch on.
     */
    val overlapRegions: List<OverlapDetector.Region> get() = frame?.overlapRegions.orEmpty()

    /**
     * Per-frame confidence for the audio of the last run, in that audio's own coordinates, and how
     * many samples one frame covers.
     *
     * Empty on the sherpa engine, which has no per-frame anything to report. Callers treat empty as
     * "weigh nothing, fall back to the timestamp rules" rather than branching on the engine.
     */
    val frameWeights: FloatArray get() = frame?.frameWeights ?: FloatArray(0)
    val samplesPerFrame: Int get() = frame?.samplesPerFrame ?: 0

    /**
     * Which cluster holds the most frame mass across `[startMs, endMs)`, and how clearly.
     *
     * `-1 to 0f` means "no opinion", which is what [DiarizationEngine.SHERPA] always answers and
     * what the frame engine answers for a span holding no attributable speech. Callers treat the two
     * the same and fall back to attribution by timestamp -- see [FrameDiarizer.wordLabel].
     */
    fun wordLabel(startMs: Long, endMs: Long): Pair<Int, Float> =
        frame?.wordLabel(startMs, endMs) ?: (-1 to 0f)
    ...
    fun load(
        ...
        provider: String = "cpu",
        /**
         * Which implementation to build. Part of the warm-pool key in both workers, so switching it
         * evicts the other engine's instances rather than handing one back.
         */
        engine: DiarizationEngine = DiarizationEngine.DEFAULT,
        /**
         * The embedding model's catalogue id, which the frame engine needs because its clustering
         * threshold is calibrated per embedder. Null falls back to the default bundle's value.
         */
        embeddingModelId: String? = null,
    ) {
        release()

        if (engine == DiarizationEngine.FRAME) {
            frame = FrameDiarizer(
                segmentationModel = segmentationModel,
                embeddingModel = embeddingModel,
                threadCount = threadCount,
                clusterThreshold = FrameDiarizer.thresholdFor(embeddingModelId),
                expectedSpeakers = expectedSpeakers,
            )
            Log.i(
                TAG,
                "frame diarizer loaded, expectedSpeakers=$expectedSpeakers, " +
                    "threshold=${FrameDiarizer.thresholdFor(embeddingModelId)}",
            )
            return
        }
    ...
    fun diarize(samples: FloatArray): List<DiarizedSegment> {
        frame?.let { return it.diarize(samples) }
    ...
    fun release() {
        ...
        runCatching { frame?.release() }
            .onFailure { Log.w(TAG, "releasing the frame diarizer failed", it) }
        frame = null
    }
