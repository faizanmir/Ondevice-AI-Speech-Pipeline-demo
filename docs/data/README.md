...
| `german_audit_bbg.ref.txt`, `german_audit_bbg.plain.txt` | A real two-voice audit recording (22:48, `german audit bbg.mp3` on the Xiaomi). The reference is the **Microsoft Teams transcript of the same meeting** (2026-08-20), converted mechanically on 2026-09-01: Teams attributes speakers from each participant's own microphone channel, so the labels are ground truth for attribution (`[S1]` = the auditee, `[S2]` = the auditor; 146 blocks, strictly alternating, one label per Teams block). The *words* are Teams' own recogniser, not a human transcript, and include 157 backchannel tokens (Mhm, Hm, Ja) that no offline model emits -- expect a WER floor of roughly 5% from those alone; speaker scoring is unaffected because it counts matched words only. Replaces the pipeline's own draft from 2026-08-31. Scored against it: Gladia's cloud diarisation puts 0.63% of matched words on the wrong speaker; our 5-min FastConformer + CAM++ run 5.77%, 2.63% with the boundary/relabel pass. |
